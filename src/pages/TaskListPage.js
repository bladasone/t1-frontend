...
export default function TaskListPage() {
    ...
    return (_jsxs(Box, { sx: {
        ...
        }, children: [tasks.map((task) => (_jsx(Box, { sx: { width: 300 }, children: _jsx(TaskItem, { task: task, onDelete: () => deleteTask(task.id), onEdit: () => navigate(`/task/${task.id}`) }) }, task.id))), _jsx(Button, { variant: "contained", onClick: () => navigate('/task/new'), sx: {
                    width: '70px',
                    height: '70px',
                    border: '1',
                    borderRadius: '35px',
